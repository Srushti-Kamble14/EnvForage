...
FORBIDDEN_PATTERNS: list[tuple[str, str]] = [
    ...
    (r"curl\s+(?:-[^\s|;&]+?\s+)*?https?://(?!(?:micro\.mamba\.pm|astral\.sh)/)\S+\s*\|\s*(?:ba)?sh", "Curl-pipe-to-shell (untrusted exec)"),
    (r"wget\s+(?:-[^\s|;&]+?\s+)*?https?://(?!(?:micro\.mamba\.pm|astral\.sh)/)\S+\s*\|\s*(?:ba)?sh", "Wget-pipe-to-shell (untrusted exec)"),
    ...
    (r"(?:iex|Invoke-Expression)\s*\(?\s*(?:iwr|Invoke-WebRequest|Invoke-RestMethod|irm|curl|wget)\s+(?!https?://astral\.sh/)", "PowerShell malicious download cradle"),
    (r"(?:iwr|Invoke-WebRequest|Invoke-RestMethod|irm|curl|wget)\s+(?!https?://astral\.sh/)\S+.*\s*\|\s*(?:iex|Invoke-Expression)", "PowerShell piped download cradle"),
